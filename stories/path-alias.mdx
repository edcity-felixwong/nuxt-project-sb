...
`TypeScript` and `Vite` do separate jobs, `TypeScript` resolves path just by `tsconfig.json` and intellisense can recognize it, but that doesn't necessary mean it can resolves at run time.
